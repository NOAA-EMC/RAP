subroutine partial_cycle_varaible_binary(iunit,ifswap,nrecs,iunit_sfc,nrecs_sfc, &
...
!  read tslb from surface file
  ...
  call to_r2i(ibuf4,tslb_sfc,(hor_size*nsig_regional))
  deallocate(ibuf4)
...
!  read tslb from background  file
  ...
  CALL mpi_file_read_at(iunit,file_offset(index+1),     &
                        ibuf4,hor_size*nsig_regional,mpi_real4,       &
                          mpi_status_ignore, ierr)
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size*nsig_regional))
  call to_r2i(ibuf4,tslb,(hor_size*nsig_regional))
  deallocate(ibuf4)
  if (ierr /= 0) then
     print*,"Error reading ", VarName," using MPIIO"
  else
...
!  read tsk from surface file
  ...
  call to_r2i(ibuf4,tsk_sfc,(hor_size))
  deallocate(ibuf4)
  if (ierr /= 0) then
     print*,"Error reading ", VarName," using MPIIO"
  else
...
!  read tsk from background  file
  ...
  CALL mpi_file_read_at(iunit,file_offset(index+1),     &
                        ibuf4,hor_size,mpi_real4,       &
                          mpi_status_ignore, ierr)
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size))
  call to_r2i(ibuf4,tsk,(hor_size))
  deallocate(ibuf4)
  if (ierr /= 0) then
     print*,"Error reading ", VarName," using MPIIO"
  else
...
!  read tsk from surface file
  ...
  CALL mpi_file_read_at(iunit_sfc,file_offset_sfc(index_sfc+1),     &
                       ibuf4,hor_size,mpi_real4,       &
                          mpi_status_ignore, ierr)
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size))
  call to_r2i(ibuf4,soilt1_sfc,(hor_size))
  deallocate(ibuf4)
  if (ierr /= 0) then
     print*,"Error reading ", VarName," using MPIIO"
  else
...
!  read tsk from background  file
  ...
  CALL mpi_file_read_at(iunit,file_offset(index+1),     &
                        ibuf4,hor_size,mpi_real4,       &
                          mpi_status_ignore, ierr)
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size))
  call to_r2i(ibuf4,soilt1,(hor_size))
  deallocate(ibuf4)
  if (ierr /= 0) then
     print*,"Error reading ", VarName," using MPIIO"
  else
...
!  read tsk from surface file
  ...
  CALL mpi_file_read_at(iunit_sfc,file_offset_sfc(index_sfc+1),     &
                       ibuf4,hor_size,mpi_real4,       &
                          mpi_status_ignore, ierr)
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size))
  call to_r2i(ibuf4,snowc,(hor_size))
  deallocate(ibuf4)
  if (ierr /= 0) then
     print*,"Error reading ", VarName," using MPIIO"
  else
...
!
  ...
  allocate(ibuf4(hor_size))
  call to_i2r(ibuf4,tsk,(hor_size))
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size))
  ...
  allocate(ibuf4(hor_size))
  call to_i2r(ibuf4,soilt1,(hor_size))
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size))
  call mpi_file_write_at(iunit,file_offset(index+1),ibuf4,  &
            hor_size,mpi_real4,mpi_status_ignore, ierr)
  deallocate(ibuf4)
  ...
  allocate(ibuf4(hor_size*nsig_regional))
  call to_i2r(ibuf4,tslb,(hor_size*nsig_regional))
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size*nsig_regional))
  call mpi_file_write_at(iunit,file_offset(index+1),ibuf4,  &
