...
!        they are MU, T, Q, U, V,  skint/sst       (for MU, need MU_B and pt_regional)
  ...
  if(l_cloud_analysis .or. nguess>0) num_mass_fields=4*lm+3+7*lm
  if(l_gsd_soilTQ_nudge) num_mass_fields=4*lm+3+2*nsig_soil+2
  if(l_cloud_analysis .and. l_gsd_soilTQ_nudge) num_mass_fields=4*lm+3+7*lm+2*nsig_soil+2
...
!    get pointer to relevant instance of cloud-related backgroud
  ...
  if(l_cloud_analysis .or. nguess>0) num_mass_fields=3+4*lm + 7*lm
  if(l_gsd_soilTQ_nudge) num_mass_fields=3+4*lm+2*nsig_soil+2
  if(l_gsd_soilTQ_nudge .and. l_cloud_analysis) num_mass_fields=3+4*lm+2*nsig_soil+2+7*lm
