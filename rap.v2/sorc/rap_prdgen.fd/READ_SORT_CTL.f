      SUBROUTINE READ_SORT_CTL (LCNTRL,MAXG,MAXF,FHR,K5PDS,MDLID,KGRIDA,
     &  TYPE,SCAL,NAMES,KGRID,IG1,KSMPR,KSMPO,ITILS,JTILS,NEARN,
     &  NFILS,NGRDS,NFLDS,JPDS5,JPDS6,JPDS7,JPDS14,JPDS15,JPDS16,
     &  JPDS19,IOUTUN,IRET)
...
c  THE PROBLEM IS THAT THE RAP HAS 2 SETS OF PRECIP FIELDS
c   AND THE CONTROL FILE HAS NO WAY TO DISTINGUISH THE TWO.
c   SO IF PRECIP IS REQUESTED, WE NEED TO ADD PLACE HOLDERS
c   FOR THE 1-HR AND RUN TOTAL ACCUMULATIONS

          if (K5PDS(1,N) .eq. 62 .or. K5PDS(1,N) .eq. 63) then
            TMPFLDS=TMPFLDS + 1
          endif
       ENDDO
       NFLDS=NFLDS+TMPFLDS 
       ...
       COUNTER = 0
       DO N=1,NFLDS - TMPFLDS 
        JPDS5(N+COUNTER)=K5PDS(1,N)
        JPDS6(N+COUNTER)=K5PDS(2,N)
        JPDS7(N+COUNTER)=K5PDS(3,N)*256+K5PDS(4,N)
        JPDS14(N+COUNTER)=FHR
        JPDS15(N+COUNTER)=0
!  ACCOUNT FOR SNOWFALL
       IF (K5PDS(1,N) .EQ. 65 .AND. K5PDS(5,N) .EQ. 4) THEN
           JPDS14(N+COUNTER)=0
           JPDS15(N+COUNTER)=FHR
       ENDIF
!  RUNOFF FIELDS ARE NOW 1-HR ACCUMULATIONS
        IF (K5PDS(1,N) .EQ. 234 .OR. K5PDS(1,N) .EQ. 235) THEN
          IF (FHR .NE. 0) THEN
           JPDS14(N+COUNTER)=FHR-1
           JPDS15(N+COUNTER)=FHR
          ELSE
           JPDS14(N+COUNTER)=FHR
           JPDS15(N+COUNTER)=FHR
          ENDIF
        ENDIF
        JPDS16(N+COUNTER)=K5PDS(5,N)
        JPDS19(N+COUNTER)=K5PDS(6,N) 
        NEARN(N+COUNTER)=K5PDS(7,N)
        DO P=1,MAXG
          KGRIDA(P,N+COUNTER)=KGRIDA(P,N)
          TYPE(P,N+COUNTER)=TYPE(P,N)
          SCAL2(P,N+COUNTER)=SCAL(P,N)
          IOUTUN2(P,N+COUNTER)=IOUTUN(P,N)
          MDLID2(P,N+COUNTER)=MDLID(P,N)
          NAMES2(P,N+COUNTER)=NAMES(P,N)
        ENDDO
        NFILS(N+COUNTER)=NFILS(N)
     ...
     &,               'WILL ASSUME YOU WANTED 1'
        ...
        IF (K5PDS(1,N).eq. 62 .or. 
     &          K5PDS(1,N) .eq. 63) THEN 
          COUNTER=COUNTER+1
          JPDS5(N+COUNTER)=K5PDS(1,N)
          JPDS6(N+COUNTER)=K5PDS(2,N)
          JPDS7(N+COUNTER)=K5PDS(3,N)*256+K5PDS(4,N)
          JPDS14(N+COUNTER-1)=0
          JPDS15(N+COUNTER-1)=FHR
          JPDS14(N+COUNTER)=FHR-1
          JPDS15(N+COUNTER)=FHR 
          JPDS16(N+COUNTER)=K5PDS(5,N)
          JPDS19(N+COUNTER)=K5PDS(6,N)
       ...
       DO M=1,NFLDS
           do P=1,MAXG
             SCAL(P,M)=SCAL2(P,M)
             IOUTUN(P,M)=IOUTUN2(P,M)
             MDLID(P,M)=MDLID2(P,M)
             NAMES(P,M)=NAMES2(P,M)
           enddo
       ENDDO
C
