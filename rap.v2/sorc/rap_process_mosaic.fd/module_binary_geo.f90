...
SUBROUTINE GEO_GET_GRID_DIM_bin(fileName,nlon,nlat,ifswap,ierr)
!
!   PRGMMR: Ming Hu          ORG: GSD        DATE: 2009-04-15
!
! ABSTRACT:
...
!
  ...
  logical :: ifswap
!  grid
  integer, intent(out) :: nlon,nlat,ierr
!
  integer(i_kind),allocatable:: start_block(:),end_block(:)
  integer(i_kind),allocatable:: start_byte(:),end_byte(:)
  integer(i_llong),allocatable:: file_offset(:)
...
!!! MPI IO
  ...
  ierr=0
  iunit=33
  open(iunit,file=trim(fileName),form='unformatted',err=220)
! Check for valid input file
  read(iunit,iostat=status_hdr)hdrbuf
  if(status_hdr /= 0) then
     write(6,*)'CONVERT_BINARY_MASS:  problem with wrfges = ',&
...
SUBROUTINE GEO_GET_GRID_bin(fileName,xlon,ylat,nlon,nlat,xland,ifswap,ierr)
!
!   PRGMMR: Ming Hu          ORG: GSD        DATE: 2009-04-15
!
! ABSTRACT:
