...
C                .      .    .
C SUBPROGRAM:    SMARTINITRAPAK    CREATES NDFD FILES 
C   PRGRMMR: MANIKIN           ORG: W/NP22     DATE: 11-10-06
C
C ABSTRACT:   THIS CODE TAKES NATIVE RAP FILES AND GENERATES
C          2.9 KM OUTPUT OVER ALASKA CONTAINING NDFD ELEMENTS
C
C PROGRAM HISTORY LOG:
C   11-10-06   G MANIKIN  - ADAPT FOR ALASKA OUTPUT 
C
...
C
     ...
     X    blr(im,jm),t2(im,jm),q2(im,jm),bli(im,jm),snod(im,jm),
...
c   SPECIAL CALL OF THE FULL DATA SET BUT WITHOUT PRECIP
     ...
     X    T2,Q2,D2,U10,V10,VEG,BLI,WETFRZ,VIS,GUST,SNOD,
...
c  CALL THE DOWNSCALING CODE 
     ...
     X      T2,Q2,D2,U10,V10,VEG,DOWNT,DOWNDEW,DOWNU,DOWNV,DOWNQ,
     X      DOWNP,TOPO)
...
c   field straight out of the RAP. 
c
...
c      than 70% and RH at bottom of BL is less than 30
     ...
     X    lcld,mcld,hcld,topo,wgust,u10,v10,refc,t2,q2,bli)
      deallocate (topo_ndfd,rough,veg,downt,downdew,downu,downv,
