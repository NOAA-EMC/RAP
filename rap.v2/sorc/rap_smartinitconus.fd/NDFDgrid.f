      subroutine NDFDgrid(validpt,psfc,zsfc,t,z,q,u,v,p,t2,q2,
     x    d2,u10,v10,veg_rap_ndfd,tnew,dewnew,unew,vnew,qnew,
     x    pnew,topo_ndfd)

      PARAMETER(IM=2145,JM=1597,LM=50)
     ...
     2      QNEW(IM,JM)
      real VEG_RAP_NDFD(IM,JM),VEG_NDFD(IM,JM),TOPO_NDFD(IM,JM)
...
c  read in 2.5 km land mask
      ...
      print *,'baopeni veg ', FNAME, IRETGI
     ...
     &               KR,KPDS,KGDS,KENS,LSKIP,LGRIB,IRGS)
        write(6,*)' IRET FROM GETGB1S ',IRGS
        IF(IRGI .NE. 0) THEN
          WRITE(6,*)' PROBLEMS ON 1ST READ OF VEG FILE SO ABORT'
          ISTAT = IRGS
          RETURN
        ENDIF
      ENDDO
     ...
     X           KPDS,KGDS,MASK,GRID,IRET)
       IF(IRET.EQ.0) THEN
         DO KK = 1, ITOT
           ...
           ELSE
             ...
             N=INT(KK/IM) + 1
           ENDIF
           IF (GRID(KK).LE. 0.) THEN
            VEG_NDFD(M,N) = 16.
           ELSE
            VEG_NDFD(M,N) = GRID(KK)
           ENDIF
         ENDDO
       ELSE
       ...
       do j=1,jm
       do i=1,im
         if (veg_rap_ndfd(i,j).eq.16.)
     1       veg_rap_ndfd(i,j) = -1.

         if (veg_rap_ndfd(i,j).ne.16..and.
     1      veg_rap_ndfd(i,j).gt.0.)
     1       veg_rap_ndfd(i,j) = 0.10

         if (veg_rap_ndfd(i,j).eq.-1.)
     1       veg_rap_ndfd(i,j) =  0.
       end do
       end do
...
C --- dew-pt at topomini
          ...
          if ((d2(I,J) - dewnew(i,j)) .gt. 15.) then
            print *, 'lt loop ', d2(i,j), dewnew(i,j)
          endif
...
C ---   Just use q at RAP 1st level in this case.
...
c        dewnew(i,j) = min(td,tnew(i,j))
         if ((d2(I,J) - dewnew(i,j)) .gt. 23.) then
            print *, 'gt loop ', i, j, d2(i,j), 
     x     dewnew(i,j), psfc(i,j), pnew(i,j), qv,
     x     q(i,j,1)
         endif
        if (k .eq. 1) then
c         u10(i,j) = u(i,j,1) - 2.
...
c   values don't get shifted around in the adjustment
       ...
       do i=1,im
         ...
         utmp(i,j)=unew(i,j)
         vtmp(i,j)=vnew(i,j)
         rough_mod(i,j) = veg_rap_ndfd(i,j)
       end do
       end do
...
c          write (6,*)'Changed water to land',i,j
       ...
       do j=1,jm
       do i=1,im
         if (veg_rap_ndfd(i,j).gt.0.05 .and.
     1      rough_mod(i,j).lt.0.05) then
C -----------------------------------------------------------------
C -- i.e.  NDFD grid-point is over WATER (per rough_mod)
C          RAP-interp grid-point is over LAND 
C -----------------------------------------------------------------
          ...
          do ibuf=1,10
           ...
           jb = min(jm,j+ibuf)
            ...
            do jw = ja,jb
              id = 1
            do iw = ia,ib,id
              if (veg_rap_ndfd(iw,jw).lt.0.05) then 
...
884       continue
          ...
          if (veg_rap_ndfd(i,j).lt.0.05 .and.
     1      rough_mod(i,j).gt.0.05) then
C -----------------------------------------------------------------
...
C -----------------------------------------------------------------
          ...
          jb = min(jm,j+ibuf)
            ...
            do jw = ja,jb
              id = 1
            do iw = ia,ib,id
              if (veg_rap_ndfd(iw,jw).gt.0.05) then
               IF (VALIDPT(I,J)) THEN
                unew(i,j) = utmp(iw,jw)
                vnew(i,j) = vtmp(iw,jw)
                tnew(i,j) = ttmp(iw,jw)
