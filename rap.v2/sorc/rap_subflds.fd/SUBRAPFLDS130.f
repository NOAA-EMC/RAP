...
C      3-HR PRESSURE TENDENCY, & 3-HR SNOWFALL ON RAPID REFRESH 130 GRID
C
...
C   CONTAINS THE PRECIP/SNOW AND PRESSURE DATA THAT WE NEED 
      ...
      ELSE IF (FHR3 .LT. 0) THEN
        ...
        LUGI3=18
        LUGB4=50
      ELSE IF (FHR .LT. 3 .AND. FHR3 .EQ. 0) THEN
        print *, 'WILL READ 2 GRIB FILES'
        LUGB=13
        LUGI=14
        LUGB3=17
...
C
      ...
      KSKIP = 0
      ENVVAR='XLFUNIT_   '
C      WRITE(ENVVAR(9:10),FMT='(I2)') LUGB
C      CALL GETENV(ENVVAR,FNAME)
      CALL BAOPEN(LUGB,'fort.13',IRETGB)
      ENVVAR='XLFUNIT_   '
C      WRITE(ENVVAR(9:10),FMT='(I2)') LUGI
C      CALL GETENV(ENVVAR,FNAME)
...
C
      ...
      JR=0
      KSKIP = 0
c      WRITE(ENVVAR(9:10),FMT='(I2)') LUGB2
c      CALL GETENV(ENVVAR,FNAME)
      CALL BAOPEN(LUGB2,'fort.15',IRETGB2)
c      WRITE(ENVVAR(9:10),FMT='(I2)') LUGI2
c      CALL GETENV(ENVVAR,FNAME)
      CALL BAOPEN(LUGI2,'fort.16',IRETGI2)
...
C
      ...
      KSKIP = 0
      ENVVAR='XLFUNIT_   '
c      WRITE(ENVVAR(9:10),FMT='(I2)') LUGB3
c      CALL GETENV(ENVVAR,FNAME)
      CALL BAOPEN(LUGB3,'fort.17',IRETGB)
      ENVVAR='XLFUNIT_   '
c      WRITE(ENVVAR(9:10),FMT='(I2)') LUGI3
c      CALL GETENV(ENVVAR,FNAME)
...
C       USE THE INFO FROM THE PREVIOUS CYCLE USED TO COMPUTE PTND
      ...
      ENVVAR='XLFUNIT_   '
c      WRITE(ENVVAR(9:10),FMT='(I2)') LUGB4
c      CALL GETENV(ENVVAR,FNAME)
