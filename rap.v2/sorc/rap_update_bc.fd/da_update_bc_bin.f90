...
!   write(*,*) 'hdrbuf=',hdrbuf(1:20)
   ...
   call count_recs_wrf_binary_file(iunit_wrfinput, ifswap,trim(wrf_input), nrecs)
!   write(*,*) 'nrecs: ', nrecs
   ...
   call inventory_wrf_binary_file(iunit_wrfinput, ifswap,trim(wrf_input), nrecs,  &
...
!   write(*,*) 'hdrbuf=',hdrbuf(1:20)
   ...
   call count_recs_wrf_binary_file(iunit_gsi, ifswap,trim(da_file), nrecs_gsi)
!        write(*,*) 'nrecs: ', nrecs_gsi
   ...
   call inventory_wrf_binary_file(iunit_gsi, ifswap,trim(da_file), nrecs_gsi,  &
...
!   write(*,*) 'hdrbuf=',hdrbuf(1:20)
   ...
   call count_recs_wrf_binary_file(iunit_bdy, ifswap,trim(wrf_bdy_file), nrecs_bdy)
!        write(*,*) 'nrecs: ', nrecs_bdy
   ...
   call inventory_wrf_binary_file(iunit_bdy, ifswap,trim(wrf_bdy_file), nrecs_bdy,  &
  ...
  call mpi_file_open(mpi_comm_world, trim(wrf_bdy_file),     &
   ...
   do n=1,num2d
     ...
     case ('MU') ;                
        ...
        allocate(ibuf4(dims(1)*dims(2)))
        CALL mpi_file_read_at(iunit_gsi,file_offset_gsi(index_gsi+1),     &
                          ibuf4,hor_size,mpi_real4,       &
                          mpi_status_ignore, ierr)
        call to_native_endianness_i4(ibuf4,(dims(1)*dims(2)))
        call to_r2i(ibuf4,field2,(dims(1)*dims(2)))
        deallocate(ibuf4)
  ...
  if ( update_lateral_bdy ) then
   ...
   do m=1,4
     ...
     if (debug) then
      ...
      field2=frst2d
      allocate(ibuf4(dims(1)*dims(2)))
      call to_i2r(ibuf4,field2,(dims(1)*dims(2)))
      call to_native_endianness_i4(ibuf4,(dims(1)*dims(2)))
   ...
   allocate(ibuf4(dims(1)*dims(2)*dims(3)))
   CALL mpi_file_read_at(iunit_gsi,file_offset_gsi(index_gsi+1),   &
                          ibuf4,hor_size*dims(2),mpi_real4,       &
                          mpi_status_ignore, ierr)
   call to_native_endianness_i4(ibuf4,(dims(1)*dims(2)*dims(3)))
   call to_r2i(ibuf4,field3,(dims(1)*dims(2)*dims(3)))
   deallocate(ibuf4)
   if (ierr /= 0) then
     print*,"Error reading ", VarName," using MPIIO"
   else
...
!tgs                  tend3d(i,k,l)=(scnd3d(i,k,l)-frst3d(i,k,l))/bdyfrq
         ...
         field3=frst3d
         allocate(ibuf4(dims(1)*dims(2)*dims(3)))
         call to_i2r(ibuf4,field3,(dims(1)*dims(2)*dims(3)))
         call to_native_endianness_i4(ibuf4,(dims(1)*dims(2)*dims(3)))
         CALL mpi_file_write_at(iunit_bdy,file_offset_bdy(index_bdy+1),     &
                          ibuf4,hor_size*dims(2),mpi_real4,       &
                          mpi_status_ignore, ierr)
