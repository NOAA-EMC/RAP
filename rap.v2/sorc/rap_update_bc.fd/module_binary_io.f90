subroutine count_recs_wrf_binary_file(in_unit,ifswap,wrfges,nrecs)
...
!   do an initial read through of a wrf binary file, and get total number of sequential file records
  ...
  character(10)   ,intent(in   ) :: wrfges
  integer(i_kind),intent(  out) :: nrecs

  character(11) cwrfges
  ...
  integer(i_llong),parameter:: lrecl=2**28_i_llong
...
subroutine inventory_wrf_binary_file(in_unit,ifswap,wrfges,nrecs, &
...
subroutine retrieve_index_bdy(index,string,varname_all,string_date, &
                               datestr_all,nrecs)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!   prgmmr: parrish          org: np22                date: 2004-11-29
!
! abstract: by examining previously generated inventory of wrf binary restart
! file,
...
!!$$$
  ...
  character(*)   ,intent(in   ) :: string_date
  character(132) ,intent(in   ) :: varname_all(nrecs)
  character(132) ,intent(in   ) :: datestr_all(nrecs)

  integer(i_kind) i

  do i=1,nrecs
     if(trim(string) == trim(varname_all(i)) .and. &
        trim(string_date) == trim(datestr_all(i))) then
        index=i
        return
     end if
  end do

  write(6,*)'RETRIEVE_INDEX:  ***PROBLEM*** reading wrf binary file, ',&
       'rec id "',trim(string),'" not found'
  index=-1
  return

end subroutine retrieve_index_bdy
...
subroutine retrieve_field(in_unit,ifswap,wrfges,out,start_block,end_block,start_byte,end_byte)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!$$$
  ...
  integer(i_kind),parameter:: lrecl=2**28
  integer(i_byte) buf(lrecl)
