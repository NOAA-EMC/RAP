...
!     
      INTEGER IEQL(IM,JSTA:JEND),IPTB(IM,JM),ITHTB(IM,JM)
      INTEGER PARCEL(IM,JSTA:JEND)      
      INTEGER KLRES(IM,JM),KHRES(IM,JM),LCL(IM,JSTA:JEND)
!     
      REAL THESP(IM,JM),PSP(IM,JSTA:JEND),CAPE20(IM,JSTA:JEND)
      REAL TVIJK   ! TV(IM,JM,LM)
...
!$omp& private(dzkl,presk,thetaa,thetap,esatp,qsatp,tvp)
        ...
        DO I=1,IM
          IF(IDX(I,J).GT.0)THEN
            ...
            TVIJK=T(I,J,L)*(1+0.608*Q(I,J,L)) 
            THETAA=TVIJK*(H10E5/PRESK)**CAPA
