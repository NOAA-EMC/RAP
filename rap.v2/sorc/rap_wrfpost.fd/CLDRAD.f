...
!     
      ...
      INTEGER IBOTT(IM,JSTA:JEND),IBOTCu(IM,JSTA:JEND),IBOTDCu(IM,JSTA:JEND)&
       ,IBOTSCu(IM,JSTA:JEND),IBOTGr(IM,JSTA:JEND), ITOPT(IM,JSTA:JEND) &
      ,ITOPCu(IM,JSTA:JEND),ITOPDCu(IM,JSTA:JEND),ITOPSCu(IM,JSTA:JEND) &
      ,ITOPGr(IM,JSTA:JEND)
      REAL EGRID1(IM,JM)
      REAL, allocatable :: EGRID2(:,:),EGRID3(:,:)
      REAL GRID1(IM,JM),GRID2(IM,JM),CLDP(IM,JSTA:JEND),              &
              CLDZ(IM,JSTA:JEND),CLDT(IM,JSTA:JEND),CLDZCu(IM,JSTA:JEND) &
            ,watericetotal(LM),watericemax,wimin               &
            ,zcldbase,zcldtop,zpbltop
      REAL :: RHBK(LM)  !RHB(IM,JM,LM) !RHBK(LM)
...
!
       ...
       if(IGET(572)>0) then
         if(grib=="grib1" )then
           ...
           ID(10)  =50
           ID(11)  =100
           CALL GRIBIT(IGET(572),LVLS(1,IGET(572)),GRID1,IM,JM)
         else if(grib=="grib2" )then
           cfld=cfld+1
...
!       THIS IS THE SFC-BASED CAPE/CIN (lowest 70 mb searched)
      IF ((IGET(032).GT.0).OR.(IGET(107).GT.0)) THEN
         IF ( (LVLS(1,IGET(032)).GT.0) .OR.                         &
            ...
            allocate(EGRID2(IM,JM),EGRID3(IM,JM))
            allocate(dummy(IM,JM),idummy(IM,JM))
            EGRID2=0.0
            EGRID3=0.0
            ITYPE = 1
	    DPBND=10.E2
