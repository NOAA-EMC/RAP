...
! DCD 4/3/13
! previously initialized PMID from sum of base-state (PB) and perturbation (P) pressure
! now initializing PMID from hydrostatic pressure (P_HYD), motivated particularly by 0h HRRR
!      VarName='PB'
!      call getVariable(fileName,DateStr,DataHandle,VarName,DUM3D,         &
!        IM+1,1,JM+1,LM+1,IM, JS,JE,LM)
!      do l = 1, lm
...
!        IM+1,1,JM+1,LM+1,IM, JS,JE,LM)
      ...
      do l = 1, lm
       do j = jsta_2l, jend_2u
        do i = 1, im
!            PMID(I,J,L)=PMID(I,J,L)+DUM3D(I,J,L)
            PMID(I,J,L)=DUM3D(I,J,L)
...
!mhu      VarName='EXTCOF55'
!mhu      call getVariable(fileName,DateStr,DataHandle,VarName,DUM3D,        &
!mhu        IM+1,1,JM+1,LM+1,IM,JS,JE,LM)
!mhu      do l = 1, lm
!mhu       do j = jsta_2l, jend_2u
!mhu        do i = 1, im
!mhu            extcof55 ( i, j, l ) = dum3d ( i, j, l )
!mhu        end do
!mhu       end do
!mhu      end do
!mhu      print*,'finish reading EXTCOF55'
...
!mhu      VarName='ISLOPE'
!mhu      call getIVariableN(fileName,DateStr,DataHandle,VarName,IDUMMY,     &
!mhu        IM,1,JM,1,IM,JS,JE,1)
!mhu       do j = jsta_2l, jend_2u
!mhu        do i = 1, im
!mhu            ISLOPE( i, j ) = idummy ( i, j )
!mhu        end do
!mhu       end do
