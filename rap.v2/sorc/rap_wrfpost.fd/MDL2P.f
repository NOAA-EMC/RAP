...
!     
      ...
      REAL Q2SL(IM,JSTA:JEND),WSL(IM,JSTA:JEND),CFRSL(IM,JSTA:JEND)
      REAL O3SL(IM,JSTA:JEND),TDSL(IM,JM)
      REAL EGRID1(IM,JM),EGRID2(IM,JM)
      REAL GRID1(IM,JM),GRID2(IM,JM)
      REAL FSL_OLD(IM,JM),USL_OLD(IM,JM),VSL_OLD(IM,JM)
      REAL ICINGFSL(IM,JSTA:JEND)
      REAL D3DSL(IM,JSTA:JEND,27),DUSTSL(IM,JSTA:JEND,5)
!
...
!
      REAL C1D(IM,JM),QW1(IM,JM),QI1(IM,JM),    &
           QR1(IM,JM),QS1(IM,JM),QG1(IM,JM)
      REAL DBZ1(IM,JSTA:JEND),FRIME(IM,JSTA:JEND),RAD(IM,JSTA:JEND)
      REAL HAINES(IM,JM)
...
!             print *,'1 USL=',USL(I,J),UH(I,J,1)
	  ...
	  IF(TTND(I,J,1).LT.SPVAL)  RAD(I,J)=TTND(I,J,1)
          ...
          IF(DUST(I,J,1,6).LT.SPVAL)DUSTSL(I,J,6)=DUST(I,J,1,6)
...
! DUST
          ...
          IF(DUST(I,J,LL,6).LT.SPVAL .AND. DUST(I,J,LL-1,6).LT.SPVAL)          &
             DUSTSL(I,J,6)=DUST(I,J,LL,6)+(DUST(I,J,LL,6)-DUST(I,J,LL-1,6))*FACT
...
! PM 2.5
         IF (IGET(723).GT.0) THEN
          IF (LVLS(LP,IGET(723)).GT.0) THEN
             DO J=JSTA,JEND
             DO I=1,IM
             ...
             if(grib=='grib1')then
              ID(1:25)=0
              ID(02)=141    ! Parameter Table 141
              CALL GRIBIT(IGET(723),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
              cfld=cfld+1
              fld_info(cfld)%ifld=IAVBLFLD(IGET(723))
              fld_info(cfld)%lvl=LVLSXML(LP,IGET(723))
              datapd(1:im,1:jend-jsta+1,cfld)=GRID1(1:im,jsta:jend)
             endif
          ENDIF
         ENDIF
