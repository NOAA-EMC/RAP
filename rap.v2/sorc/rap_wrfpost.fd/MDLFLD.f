...
!
      ...
      INTEGER,allocatable :: ICBOT(:,:),ICTOP(:,:) 

!     
!     DECLARE VARIABLES.
!     
      LOGICAL NORTH,NEED(IM,JM), NMM_GFSmicro
      REAL,ALLOCATABLE :: FI1D(:,:),FR1D(:,:),FS1D(:,:), &
                          Zfrz(:,:), DBZ1(:,:),DBZR1(:,:),DBZI1(:,:), &
                          DBZC1(:,:), &
                          CUREFL_S(:,:), CUREFL(:,:), CUREFL_I(:,:), &
                          NLICE1(:,:), C1D(:,:)
      REAL,ALLOCATABLE :: EGRID5(:,:),EGRID6(:,:),EGRID7(:,:)
      REAL :: EGRID1(IM,JM),EGRID2(IM,JM),EGRID3(IM,JM), &
              EGRID4(IM,JM)
      REAL,ALLOCATABLE :: EL0(:,:) 
      REAL,ALLOCATABLE :: QW1(:,:),QI1(:,:),QR1(:,:),QS1(:,:),QG1(:,:)
       REAL :: GRID1(IM,JM),P1D(IM,JM),T1D(IM,JM),Q1D(IM,JM)
       REAL :: GRID2(IM,JM)
!
...
!      REAL QRAIN1(IM,JM),QSNO1(IM,JM), refl(im,jm),QG1(IM,JM)    &
      REAL refl(im,JSTA:JEND)    &
           ,refl1km(IM,JSTA:JEND),refl4km(IM,JSTA:JEND)
      REAL RH(IM,JM)
!     
...
!        print*,'DTQ2 in MDLFLD= ',DTQ2
        allocate(ICBOT(IM,JM),ICTOP(IM,JM))
...
!
      IF(imp_physics==5 .or. NMM_GFSmicro)THEN
       allocate(Zfrz(IM,JM))
       allocate(DBZ1(IM,JM))
       allocate(DBZR1(IM,JM))
       allocate(DBZI1(IM,JM))
       allocate(DBZC1(IM,JM))
       allocate(FI1D(IM,JM))
       allocate(FR1D(IM,JM))
       allocate(FS1D(IM,JM))
       allocate(CUREFL_S(IM,JM))
       allocate(CUREFL(IM,JM))
       allocate(CUREFL_I(IM,JM))
       allocate(NLICE1(IM,JM))
       allocate(C1D(IM,JM))
       allocate(QW1(IM,JM),QI1(IM,JM),QR1(IM,JM),QS1(IM,JM))
...
!     &'sample QQR DEN,DBZ= ',QQR(I,J,L),DENS,DBZ(I,J,L)
     ...
     &         DBZI(I,J,L)=10.0*LOG10(DBZI(I,J,L)) ! DBZ
         ...
         ENDDO
!
       deallocate(Zfrz,DBZ1,DBZR1,DBZI1,DBZC1)
       deallocate(FI1D,FR1D,FS1D)
       deallocate(CUREFL_S,CUREFL,CUREFL_I)
       deallocate(NLICE1,C1D)
       deallocate(QW1,QI1,QR1,QS1)
...
!           TURBULENT KINETIC ENERGY ON MDL SURFACES.
	        ...
	        LL=LM-L+1
                ...
                ENDDO
                GRID1=0.0
                write(*,*) 'TKE=',maxval(GRID1), minval(GRID1)
                if(grib=="grib1") then
                  ID(1:25) = 0
...
!
! PM 2.5
           IF (IGET(720).GT.0) THEN
            IF(LVLS(L,IGET(720)).GT.0) THEN
              LL=LM-L+1
               ID(1:25) = 0
               DO J=JSTA,JEND
               DO I=1,IM
                 GRID1(I,J) = DUST(I,J,LL,6)
               ENDDO
               ENDDO
               ID(02)=141    ! Parameter Table 141
               if(grib=="grib1") then
                 ID(11) = L
                 CALL GRIBIT(IGET(720),L,GRID1,IM,JM)
               else if(grib=="grib2" )then
                 cfld=cfld+1
                 fld_info(cfld)%ifld=IAVBLFLD(IGET(720))
                 fld_info(cfld)%lvl=LVLSXML(L,IGET(720))
                 datapd(1:im,1:jend-jsta+1,cfld)=grid1(1:im,jsta:jend)
               endif
            END IF
           ENDIF
! PM 10
           IF (IGET(721).GT.0) THEN
            IF(LVLS(L,IGET(721)).GT.0) THEN
              LL=LM-L+1
               ID(1:25) = 0
               DO J=JSTA,JEND
               DO I=1,IM
                 GRID1(I,J) = DUST(I,J,LL,7)
               ENDDO
               ENDDO
               ID(02)=141    ! Parameter Table 141
               if(grib=="grib1") then
                 ID(11) = L
                 CALL GRIBIT(IGET(721),L,GRID1,IM,JM)
               else if(grib=="grib2" )then
                 cfld=cfld+1
                 fld_info(cfld)%ifld=IAVBLFLD(IGET(721))
                 fld_info(cfld)%lvl=LVLSXML(L,IGET(721))
                 datapd(1:im,1:jend-jsta+1,cfld)=grid1(1:im,jsta:jend)
               endif
            END IF
           ENDIF
! SO2
           IF (IGET(722).GT.0) THEN
            IF(LVLS(L,IGET(722)).GT.0) THEN
              LL=LM-L+1
               ID(1:25) = 0
               DO J=JSTA,JEND
               DO I=1,IM
                 GRID1(I,J) = DUST(I,J,LL,8)
               ENDDO
               ENDDO
               ID(02)=141    ! Parameter Table 141
               if(grib=="grib1") then
                 ID(11) = L
                 CALL GRIBIT(IGET(722),L,GRID1,IM,JM)
               else if(grib=="grib2" )then
                 cfld=cfld+1
                 fld_info(cfld)%ifld=IAVBLFLD(IGET(722))
                 fld_info(cfld)%lvl=LVLSXML(L,IGET(722))
                 datapd(1:im,1:jend-jsta+1,cfld)=grid1(1:im,jsta:jend)
               endif
            END IF
           ENDIF
...
!
      IF (IGET(180).GT.0) THEN
        RDTPHS=1./DTQ2
!
!--- Needed values at 1st level above ground  
!            (Jin, '01; Ferrier, Feb '02)
!
        allocate(QW1(IM,JM),QI1(IM,JM),QR1(IM,JM),QS1(IM,JM),QG1(IM,JM))
        DO J=JSTA,JEND
          DO I=1,IM
            LLMH=NINT(LMH(I,J))
            Q1D(I,J)=Q(I,J,LLMH)
...
!                  SNORATE=SR(I,J)*CUPPT(I,J)/(TRDLW*3600.)
	   ...
	   END IF
        ...
        ENDDO
!
!-- Visibility using Warner-Stoelinga algorithm
!  (Jin, '01)
!
        ii=im/2
        jj=(jsta+jend)/2
...
!
	...
	END DO  
	 ...
	 CALL GRIBIT(IGET(180),LM,GRID1,IM,JM)
       ...
       deallocate(QW1,QI1,QR1,QS1,QG1)
      ENDIF
...
!
      IF ( (IGET(111).GT.0) .OR. (IGET(146).GT.0) .OR.         &
           (IGET(147).GT.0) ) THEN
!     
         allocate(EL0(IM,JM),EGRID5(IM,JM))
...
!mp     have model layer heights (ZMID, known) so we can average the winds (known) up to the PBLH (known)
            ...
            IF ( (IGET(389).GT.0) .OR. (IGET(454).GT.0) ) THEN
                allocate(EGRID5(IM,JM),EGRID6(IM,JM),EGRID7(IM,JM))
	        DO J=JSTA,JEND
		 DO I=1,IM
...
!        write(0,*) 'min, max of GRID2 (v comp transport wind): ', minval(grid2),maxval(grid2)
               ...
               END IF
               deallocate(EGRID5,EGRID6,EGRID7)
            ENDIF
!
