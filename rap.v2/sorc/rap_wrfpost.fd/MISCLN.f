...
!     
      ...
      REAL PKU1
      REAL, allocatable :: RH4710(:,:),RH4796(:,:),RH1847(:,:),&
                           RH8498(:,:),QM8510(:,:)
      REAL, allocatable :: UST(:,:),VST(:,:)
      ...
      REAL, allocatable :: T7D(:,:,:),Q7D(:,:,:),U7D(:,:,:),V6D(:,:,:) &
          ,P7D(:,:,:),ICINGFD(:,:,:)
      REAL, allocatable :: MAXWP(:,:),MAXWZ(:,:),MAXWU(:,:), &
                           MAXWV(:,:),MAXWT(:,:)
!      REAL HELI(IM,JM,2)
...
!
	...
	write(6,*) 'call FDLVL with ITYPEFDLVL: ', ITYPEFDLVL
         allocate(T7D(IM,JM,NFD),Q7D(IM,JM,NFD),U7D(IM,JM,NFD), &
                  V6D(IM,JM,NFD),P7D(IM,JM,NFD),ICINGFD(IM,JM,NFD))
