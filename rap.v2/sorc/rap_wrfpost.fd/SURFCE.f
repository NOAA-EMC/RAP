...
!GSD
      ...
      REAL,allocatable:: ECAN(:,:),EDIR(:,:),ETRANS(:,:),ESNOW(:,:) &
           ,SMCDRY(:,:),SMCMAX(:,:)
      REAL,allocatable:: RSMIN(:,:),SMCREF(:,:)     &
       ,RCS(:,:),RCQ(:,:),RCT(:,:),RCSOIL(:,:)  &
       ,GC(:,:)     
...
!     TOTAL SOIL MOISTURE
      IF (IGET(036).GT.0) THEN
         ...
         ID(10) = 0
         ID(11) = 200
         write(*,*) '036=',maxval(GRID1),minval(GRID1)
	 If(grib=='grib1') then
...
!         minval(cmc(1:im,jsta:jend))
      IF ( IGET(228).GT.0 .OR. IGET(229).GT.0      &
       ...
       .OR.IGET(232).GT.0 .OR. IGET(233).GT.0) THEN
        ...
        allocate(ECAN(IM,JM),EDIR(IM,JM),ETRANS(IM,JM),ESNOW(IM,JM))
        allocate(SMCDRY(IM,JM),SMCMAX(IM,JM))
...
! WRF ARW has no POTEVP field. So has to block out RAPR
	...
	If(grib=='grib1') then
	 ...
	 ID(02)= 130
      ...
      deallocate(ECAN,EDIR,ETRANS,ESNOW,SMCDRY,SMCMAX)
...
! ONLY OUTPUT NEW LSM FIELDS FOR NMM AND ARW BECAUSE RSM USES OLD SOIL TYPES
     ...
     & .OR. IGET(241).GT.0 .OR. IGET(254).GT.0 ) THEN
         allocate(RSMIN(IM,JM),SMCREF(IM,JM),GC(IM,JM))
         allocate(RCS(IM,JM),RCQ(IM,JM),RCT(IM,JM),RCSOIL(IM,JM))
...
!	 print*,'outputting leaf area index= ',XLAI
	  ...
	  ID(02)= 130
         ...
         deallocate(RSMIN,SMCREF,GC)
         deallocate(RCS,RCQ,RCT,RCSOIL)
