...
#  rm -f ${real_prefix}.d01.${time_str}${real_suffix}
  ln -s ${INPUT_DATAROOT}/${real_prefix}.d01.${time_str}${real_suffix}
...
# Run real
export pgm=$EXECrap/rap_real
. prep_step

startmsg
##${MPIRUN} -np ${REALPROC} ${REAL}
mpirun.lsf $pgm < namelist.input 
export err=$?;err_chk
...
outputfile=${RAPBC}/wrfinput_d01_${start_year}-${start_month}-${start_day}_${start_hour}:00:00
echo "${outputfile}"
cp wrfinput_d01 ${outputfile}
cp wrfinput_d01 wrfinput_d01_${start_year}-${start_month}-${start_day}_${start_hour}:00:00
...
# create tar file of boundary files that are stored so that if a current set of
#   boundary files is not usable, the tar ball can be untarred and moved to nwges
mkdir tar
cp wrfbdy_d01.* tar/.
