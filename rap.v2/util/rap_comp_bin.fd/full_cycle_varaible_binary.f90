subroutine full_cycle_varaible_binary_comp(VarName,ifswap,iunit,nrecs,iunit_sfc,nrecs_sfc, &
...
!
  ...
  real(r_single),allocatable:: field3a(:,:,:)
  integer,allocatable:: ifield3a(:,:,:)
...
!
  ...
  if(trim(memoryorder_all(index))=='XZY') then
     ...
     allocate(field3a(nlon_regional,nsig_regional,nlat_regional))
     if(if_integer) allocate(ifield3a(nlon_regional,nsig_regional,nlat_regional))
     ...
     allocate(field3a(nlon_regional,nlat_regional,nsig_regional))
     if(if_integer) allocate(ifield3a(nlon_regional,nlat_regional,nsig_regional))
  ...
  allocate(ibuf4(hor_size*nsig_regional))
  call mpi_file_read_at(iunit,file_offset(index+1),ibuf4,  &
            hor_size*nsig_regional,mpi_real4,mpi_status_ignore, ierr)
  if(ifswap) call to_native_endianness_i4(ibuf4,(hor_size*nsig_regional))
  call to_r2i(ibuf4,field3a,(hor_size*nsig_regional))
  deallocate(ibuf4)

  if(if_integer) ifield3a=field3a
  if (ierr /= 0) then
     ...
     if(trim(memoryorder_all(index))=='XZY') then
       do k=1,nsig_regional
         if(if_integer) then
           write(6,*) 'k, max, min, mid ',k,maxval(ifield3a(:,k,:)),minval(ifield3a(:,k,:)), &
                    ifield3a(nlon_regional/2,k,nlat_regional/2)
         else
           write(6,*) 'k, max, min, mid ',k,maxval(field3a(:,k,:)),minval(field3a(:,k,:)), &
                    field3a(nlon_regional/2,k,nlat_regional/2)
         endif
       enddo
     else
       do k=1,nsig_regional
         if(if_integer) then
           write(6,*) 'k, max, min, mid ',k,maxval(ifield3a(:,:,k)),minval(ifield3a(:,:,k)), &
                    ifield3a(nlon_regional/2,nlat_regional/2,k)
         else
           write(6,*) 'k, max, min, mid ',k,maxval(field3a(:,:,k)),minval(field3a(:,:,k)), &
                    field3a(nlon_regional/2,nlat_regional/2,k)
         endif
       enddo
     endif
  end if
     ...
     write(6,*)' DIFFERENCE '
     ifield3a=ifield3a-ifield3
     field3a=field3a-field3
     if(trim(memoryorder_all(index))=='XZY') then
       do k=1,nsig_regional
         if(if_integer) then
           write(6,*) 'k, max, min, mid ',k,maxval(ifield3a(:,k,:)),minval(ifield3a(:,k,:)), &
  ...
  deallocate(field3)
  deallocate(field3a)
  if(if_integer) deallocate(ifield3a)

end subroutine full_cycle_varaible_binary_comp
