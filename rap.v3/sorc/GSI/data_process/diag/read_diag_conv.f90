...
!
!  This is to read GSI conventional diagnositic data from a combined file diag_cov*
!
!      ps
!      t
!      q
...
!
  ...
  namelist/iosetup/ dirname, outfilename,ndate,nloop
!
! output variables
!
  character(len=3)  :: var
  real :: rlat,rlon,rprs,robs1,rdpt1,robs2,rdpt2,ruse,rerr,rhght
  real :: rdhr, ddiff
  character(8) :: tmp
...
!  misc.
!
  character(10) :: cdate                    ! processor numbers used to run GSI
...
!
  ...
  open(42, file=trim(outfilename)//".conv_"//trim(cloop(iloop)),IOSTAT=ios)
  if(ios > 0 ) then
       write(*,*) 'LOOP:',iloop,' cannot open file ', trim(outfilename)//".conv_"//trim(cloop(iloop))
       cycle
  else
       write(*,*) 'LOOP:',iloop,' open file ', trim(outfilename)//".conv_"//trim(cloop(iloop))
  endif
!
  DO ipe=0, 0
!  DO ipe=0, numpe-1
     infilename=trim(dirname)//"/diag_conv_"//trim(cloop(iloop))//"."//trim(cdate)
     OPEN (17,FILE=trim(infilename),STATUS='OLD',IOSTAT=ios,ACCESS='SEQUENTIAL',  &
             FORM='UNFORMATTED' )
     if(ios > 0 ) then
       write(*,*) ' no diag file availabe at', trim(infilename)
       cycle
     endif
     if(ipe==0) then
...
100  continue
     ...
     if (ii > 0) then
          ...
          do i=1,ii
             ...
             rhght=rdiagbuf(7,i)    ! observation height (m)
...
! check use
             if(iuse == -1 ) then          
                ...
                elseif( iusev  == 101 ) then 
                  iuse = -5          ! bad quality flag 
                elseif( iusev  == 102 ) then 
                  iuse = -6          ! bad pressure quality flag 
                else
...
!   When the data is pw, replase the rprs to -999.0 **/
           ...
           if(rhght > 30000.0 .or. rhght < 0.0 ) then
                 rhght=-999.0
           endif
           tmp = cdiagbuf(i)
           iflg = 0
...
!  write out result for one variable on one pitch
           if (var .ne. " uv") then 
              write (42,'(A3," @ ",A8," : ",I3,F6.2,F8.2,F8.2,   &
                    F8.2,I3,F8.2,F8.2,F9.1,I5)') var,tmp,itype, rdhr,   &
                    rlat,rlon,rprs,iuse,robs1,rdpt1,rhght ! ,rerr,iusev
           else
              write (42,'(A3," @ ",A8," : ",I3,F6.2,F8.2,F8.2,   &
...
!  ** When the data is uv, additional output is needed **/
           if (var == " uv") then 
             robs2=rdiagbuf(20,i)
             rdpt2=rdiagbuf(21,i);
             write (42,'(F8.2,F8.2,F9.1,I5)') robs2, rdpt2,rhght !,rerr,iusev
           endif
