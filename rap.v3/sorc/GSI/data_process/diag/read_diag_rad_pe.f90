...
!
  ...
  namelist/iosetup/ dirname, outfilename,numpe,nloop,instrument
!
! output variables
!
  real :: rlat,rlon,rprs
...
!
    DO iloop = 1, 5
      if(nloop(iloop) > 0 ) then
         ...
         ifirst = 1
         open(12, file=trim(outfilename)//"."//trim(this_instrument)//"_"//trim(cloop))
!
      
         DO ipe=0, numpe-1
!         DO ipe=20,20
             write(cipe,'(I4.4)')  ipe
             infilename=trim(dirname)//"/pe"//trim(cipe)//"."//trim(this_instrument)//"_"//trim(cloop)
