...
!
  call update_SNOWICE_binary_mass(snowiceRR, xlandRR, luseRR, nlonRR, nlatRR,xlandIMS)
!
...
!                  LU_INDEX
  ...
  call retrieve_field(iunit,trim(filename),field2,start_block(index+1),end_block(index+1),&
                               start_byte(index+1),end_byte(index+1))
  luse=field2
  write(6,*)' MPIIO: landmask, max, min=', maxval(luse),minval(luse)
!
!
  close(iunit)
  deallocate(field2)
