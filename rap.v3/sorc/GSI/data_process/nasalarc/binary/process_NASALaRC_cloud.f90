...
!
  ...
  use gridmod_gsimap ,only : nlon,nlat,init_general_transform,tll2xy
...
!
  ...
  call GEO_GET_GRID_DIM_bin(geofile,NLON,NLAT)
  write(*,*) 'NLON,NLAT',NLON,NLAT

  call GEO_GET_MAP_bin(geofile, userDX, userDY, CEN_LAT, CEN_LON, &
...
!
  ...
  call GEO_GET_GRID_bin(geofile,xlon,ylat,Nlon,Nlat,xland)
  deallocate(xland)
...
!           call latlon_to_ij(proj_stack, 90.0-lat_l(ipt), lon_l(ipt),xc,yc)
!         else
           rlon=lon_l(ipt)*deg2rad
           rlat=lat_l(ipt)*deg2rad
           call tll2xy(rlon,rlat,xc,yc)
!           write(*,*) lon_l(ipt),90.0-lat_l(ipt),xc,yc
!         endif
...
!  write out results
!
  call write_bufr_NASALaRC(nlon,nlat,index,w_pcld,w_tcld,w_frac,w_lwp,nlev_cld)
!
