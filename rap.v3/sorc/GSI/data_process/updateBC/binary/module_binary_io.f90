...
!   do an initial read through of a wrf binary file, and get total number of sequential file records
  ...
  character(len=10)   ,intent(in   ) :: wrfges
...
!  call openfileread (in_unit, ierr, cwrfges)
  open(in_unit,file=trim(cwrfges),access='direct',recl=lrecl)
  nrecs=0
  missing=-9999_i_long
  nextbyte=0_i_llong
  locbyte=lrecl
...
subroutine next_buf_gsi(in_unit,buf,nextbyte,locbyte,thisblock,lrecl,nreads,lastbuf)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
end subroutine next_buf_gsi

subroutine retrieve_index_bdy(index,string,varname_all,string_date,datestr_all,nrecs)
!$$$  subprogram documentation block
!                .      .    .                                       .
