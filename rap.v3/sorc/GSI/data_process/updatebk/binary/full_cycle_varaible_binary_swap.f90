subroutine full_cycle_varaible_binary_swap(VarName,iunit,nrecs,iunit_sfc,nrecs_sfc, &
...
!
  ...
  call retrieve_index_swap(index_sfc,VarName,varname_all_sfc,nrecs_sfc)
  if(index_sfc < 0) then
  ...
  CALL mpi_file_read_at_swap4(iunit_sfc,file_offset_sfc(index_sfc+1),     &
...
!  call mpi_file_read_at(iunit,file_offset(index+1),field3,  &
!            hor_size*nsig_regional,mpi_real4,mpi_status_ignore, ierr)
  call mpi_file_write_at(iunit,file_offset(index+1),field3,  &
            hor_size*nsig_regional,mpi_real4,mpi_status_ignore, ierr)
  if (ierr /= 0) then
     ...
     if(trim(memoryorder_all(index))=='XZY') then
       do k=1,nsig_regional
         write(6,*) 'bk, max, min, mid ',k,maxval(field3(:,k,:)),minval(field3(:,k,:)), &
                    field3(nlon_regional/2,k,nlat_regional/2)
       enddo
     else
       do k=1,nsig_regional
...
end subroutine full_cycle_varaible_binary_swap
