subroutine count_recs_wrf_binary_file_swap(in_unit,wrfges,nrecs)
...
end subroutine count_recs_wrf_binary_file_swap

subroutine inventory_wrf_binary_file_swap(in_unit,wrfges,nrecs, &
...
!    get length of next record
     ...
     do i=2,8
        ...
        if(locbyte > lrecl) call next_buf(in_unit,buf,nextbyte,locbyte,thisblock,lrecl,nreads,lastbuf)
        ii=(i-1)/4
        iii=ii*4 + (4-mod(i-1,4))
        hdrbuf4(iii)=buf(locbyte)
     end do
...
end subroutine inventory_wrf_binary_file_swap
subroutine next_buf_gsi_swap(in_unit,buf,nextbyte,locbyte,thisblock,lrecl,nreads,lastbuf)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
end subroutine next_buf_gsi_swap

subroutine retrieve_index_swap(index,string,varname_all,nrecs)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
end subroutine retrieve_index_swap

subroutine retrieve_field_swap(in_unit,wrfges,out,start_block,end_block,start_byte,end_byte)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!$$$
  ...
  do k=start_block,end_block
     ...
     do i=ibegin,iend
        ii=ii+1
        iii=(ii-1)/4
        iiii=iii*4 + (4-mod(ii-1,4))
        out(iiii)=buf(i)
     end do
  end do
  close(in_unit)
  
end subroutine retrieve_field_swap


SUBROUTINE int_get_ti_header_integer_swap( hdrbuf, hdrbufsize, itypesize, typesize, &
...
!write(0,*)'int_get_ti_header_integer ',TRIM(Element), Data(1)
  hdrbufsize = hdrbuf(1)
  RETURN
END SUBROUTINE int_get_ti_header_integer_swap

SUBROUTINE int_get_ti_header_real_swap( hdrbuf, hdrbufsize, itypesize, typesize, &
                              DataHandle, Element, Data, Count, code )
!<DESCRIPTION>
!<PRE>
...
!write(0,*)'int_get_ti_header_real ',TRIM(Element), Data(1)
  hdrbufsize = hdrbuf(1)
  RETURN
END SUBROUTINE int_get_ti_header_real_swap

SUBROUTINE int_get_ti_header_char_swap( hdrbuf, hdrbufsize, itypesize, &
...
!
  ...
  RETURN
END SUBROUTINE int_get_ti_header_char_swap

SUBROUTINE int_get_write_field_header_swap ( hdrbuf, hdrbufsize, ftypesize, &
                                        DataHandle , DateStr , VarName , FieldType ,                 &
                                        DomainDesc , MemoryOrder , Stagger , DimNames ,              &
...
!Local
  ...
  RETURN
END SUBROUTINE int_get_write_field_header_swap

SUBROUTINE int_unpack_string_swap ( str, buf, n )
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!Local
  ...
  ENDDO
  n = strlen + 1
END SUBROUTINE int_unpack_string_swap
