...
!$$$  end documentation block
  ...
  idate=0
  if(trim(dtype) == 'tcp' .or. trim(dtype) == 'rad_ref' .or. trim(filename) == 'tldplrso')return
...
subroutine gsdcldana_read_obs(ndata,mype)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!            Process radar reflectivity Mosaic
                ...
                call read_RadarRef_mosaic_bin(nread,npuse,infile,obstype,lunout,twind,sis)
...
!   End of routine
    return
end subroutine gsdcldana_read_obs


end module gsdcldana_read_obsmod
