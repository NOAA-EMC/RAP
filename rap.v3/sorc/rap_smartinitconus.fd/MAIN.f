...
C                .      .    .
C SUBPROGRAM:    SMARTINITRAPCONUS    CREATES NDFD FILES 
...
C   2011-09-10  G MANIKIN  - CODE REVISED FOR RAPID REFRESH 
C
...
C
     ...
     X    lcld(im,jm),mcld(im,jm),hcld(im,jm),sky(im,jm),
     X    wx(im,jm),thunder(im,jm),vis(im,jm),refc(im,jm),
     X    topo(im,jm),wgust(im,jm),pmid(im,jm,maxlev),snod(im,jm))
     ...
     X    mgtrans(im,jm),lal(im,jm),slp(im,jm),ceil(im,jm))
...
c   SPECIAL CALL OF THE FULL DATA SET BUT WITHOUT PRECIP
     ...
     X    T2,Q2,D2,U10,V10,COAST,BLI,WETFRZ,VIS,
     X    GUST,SNOD,REFC,TCLD,LCLD,MCLD,HCLD,BASEZ,CEIL,
     X    SLP,DATE,FHR)

c  CALL THE DOWNSCALING CODE 
       CALL NDFDgrid(VALIDPT,PSFC,ZSFC,T,HGHT,Q,UWND,VWND,PMID,
     X      T2,Q2,D2,U10,V10,COAST,DOWNT,DOWNDEW,DOWNU,DOWNV,DOWNQ,
     X      DOWNP,TOPO)
...
c      than 70% and RH at bottom of BL is less than 30
     ...
     X    lcld,mcld,hcld,topo,wgust,u10,v10,refc,t2,q2,bli)
      deallocate (topo_ndfd,rough,coast,downt,downdew,downu,downv,downq,
     X    downp,gridwx,mixhgt,mgtrans,lal,basez,snod,slp,ceil)
