...
!        allocate(gfld%bmap(1200*1200))
      ...
      ALLOCATE (APCP1(NUMVAL),CAPCP1(NUMVAL),NCAPCP1(NUMVAL),SNOW1(NUMVAL), &
...
!   NON-CONVECTIVE PRECIP 
        ...
        write(0,*) 'set NCAPCP1 to zero'
        NAPCP1=0.

        else

        write(0,*) 'size(NCAPCP1): ', size(NCAPCP1)
        write(0,*) 'size(gfld%fld): ', size(gfld%fld)

        NCAPCP1=gfld%fld

        do K=1,200
        PDS_NCPCP_HOLD_EARLY(K)=gfld%ipdtmpl(K)
        enddo
...
!=======================================================
      ...
      ALLOCATE (APCP2(NUMVAL),CAPCP2(NUMVAL),NCAPCP2(NUMVAL),SNOW2(NUMVAL), &
...
!     ACCUMULATED NON-CONVECTIVE PRECIP 
        ...
        do K=1,200
        PDS_NCPCP_HOLD(K)=gfld%ipdtmpl(K)
        enddo
...
!=======================================================
      ALLOCATE (APCPOUT(NUMVAL),CAPCPOUT(NUMVAL),NCPCPOUT(NUMVAL),SNOWOUT(NUMVAL), &
...
!        endif
      ...
      KPDS(5)=62
      print *, 'writing non-cnvct precip', KPDS(5),KPDS(14),KPDS(15), &
          LUGB6,MAXVAL(NCPCPOUT)
