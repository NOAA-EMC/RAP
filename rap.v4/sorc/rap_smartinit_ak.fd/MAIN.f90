...
!                .      .    .
! SUBPROGRAM:    SMARTINITRAPCONUS    CREATES NDFD FILES 
...
!
      PARAMETER(IM=1649,JM=1105,MAXLEV=50)
...
!      LOGICAL*1 MASK(ITOT), MASK2(ITOT), VALIDPT(IM,JM) 
...
!
      ...
      REAL, ALLOCATABLE :: HGHT(:,:,:), T(:,:,:), &
        ...
        DIRTRANS(:,:),T1(:,:),D2(:,:), &
        WX(:,:),THUNDER(:,:),VIS(:,:),GUST(:,:), &
        TOPO(:,:),WGUST(:,:),MIXHGT(:,:),SNOD(:,:), &
        MGTRANS(:,:),LAL(:,:),BASEZ(:,:),SLP(:,:), &
        CEIL(:,:),SFCR(:,:)   
      REAL, ALLOCATABLE :: TOPO_NDFD(:,:),ROUGH(:,:),COAST(:,:), &
        DOWNT(:,:),DOWNDEW(:,:),DOWNU(:,:),DOWNP(:,:), &
        DOWNV(:,:),DOWNQ(:,:),GRIDWX(:,:),GFIELD(:)
      TYPE (GINFO) :: GDIN
      ...
      allocate (hght(im,jm,maxlev),t(im,jm,maxlev),q(im,jm,maxlev), &
         ...
         blr(im,jm),t2(im,jm),q2(im,jm),bli(im,jm),basez(im,jm), &
         ...
         topo(im,jm),wgust(im,jm),pmid(im,jm,maxlev),snod(im,jm)) 
      allocate (topo_ndfd(im,jm),rough(im,jm),coast(im,jm), &
          downt(im,jm), &
         downdew(im,jm),downu(im,jm),downv(im,jm),downq(im,jm), &
         downp(im,jm),gridwx(im,jm),mixhgt(im,jm), &
         mgtrans(im,jm),lal(im,jm),slp(im,jm),ceil(im,jm),gfield(itot), &
         sfcr(im,jm))
...
!        NUMLEV=GDIN%KMAX
        ...
        CALL GETGRIB(PSFC,ZSFC,PMID,HGHT,T,Q,UWND,VWND, &
         T2,Q2,D2,U10,V10,COAST,BLI,WETFRZ,VIS, &
         GUST,SNOD,REFC,TCLD,LCLD,MCLD,HCLD,BASEZ,CEIL, &
         SLP,SFCR,VALIDPT,DATE,FHR,GDIN,GFLD,GFLD8)

!        LUGB=11
!        LUGI=12
!        write(0,*) 'call RDHDRS MAIN'
!        CALL RDHDRS_g2(LUGB,LUGI,IGDNUM,GDIN,NUMVAL)
!  CALL THE DOWNSCALING CODE 
       CALL NDFDgrid(VALIDPT,PSFC,ZSFC,T,HGHT,Q,UWND,VWND,PMID, &
           T2,Q2,D2,U10,V10,COAST,DOWNT,DOWNDEW,DOWNU,DOWNV,DOWNQ, &
           DOWNP,TOPO)
...
!      Write visibility to grib2
       ...
       CALL set_scale(gfld,DEC)
! Additional precision needed for Alaska
!      gfld%idrtmpl(2)=0
!      gfld%idrtmpl(3)=-2
!      gfld%idrtmpl(4)=10
       CALL PUTGB2(71,GFLD,IRET)
...
!      Write best lifted index to grib2
      ...
      deallocate (topo_ndfd,rough,coast,downt,downdew,downu,downv, &
          downq, gfield, &
         downp,gridwx,mixhgt,mgtrans,lal,basez,snod,slp,ceil)
