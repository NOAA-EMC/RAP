...
cBinbin      PARAMETER (LM=50,NPNT=19,NSTAT=1400 !For eta
...
cBinbin     &, ROG=287.04/9.8)
     ...
     &, LCL1ML=13,LCL1SL=50,LCL1SOIL=2  
     &, LCL1ML1=14,LCL1SL1=58,ROG=287.04/9.8
...
cBZHOU      LRECPR=4*(8+9+LCL1ML1*LM1+LCL1SL1)        !for RSM/ETA
 ...
 33   CONTINUE
      ...
      print *, 'about to read record ', NREC
     ...
     &   ISTAT,CISTAT,(FPACK(N),N=1,9),(FPACK(N),N=10,FPACK(7))
      ...
      print *, 'READ ', IHRST,IDATE,IFCST, ISTAT, FPACK(1), FPACK(10)
      IF(ISTAT.EQ.724030) THEN
        do MM = 1,709
         print *, 'fpack ', MM, FPACK(MM), IFCST
        enddo
      END IF
...
c       QSNO=SNOCON*TERM4*TERM5*TERM6
       ...
       print *, 'CALVIS ', HOVI, QV, QCD, QRAIN, QICE, QSNO, TT, PPP
...
C      NLEN IS THE STARTING POINT - GSM
          NLEN = FPACK(7)         
          print *, 'NLEN ', NLEN
          print *, 'NWORD ', NWORD
          FPACK(NLEN+7) = CDBP     
          ...
          FPACK(6) = FPACK(6) + 8  !9 new variables but write over 1
          FPACK(7) = 7 + FPACK(5)*FPACK(4) + FPACK(6)
          print *, 'NEW FPACK7 ', FPACK(7)
...
C    IT IS CURRENTLY WRITTEN IN REVERSE ORDER.
        ...
        IF (ISTAT .eq. 724030 .and. JHR .eq. 22) THEN
         DO M=1,765
...
C
        CLIST1(1)=' '
C       CHANGED FROM NSTAT_TRUE to deal with varying number of 
C       forecast hours. parm.em has to have exact number of 
C       stations.
        DO I=1,NSTAT
         NLVL(1)=LLMH(I)
         NLVL(2)=NSOIL
