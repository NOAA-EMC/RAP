...
mkdir -p $DATA/smart
cd $DATA/smart
/gpfs/gp1/nco/ops/nwprod/util/ush/setup.sh

echo $fhr
export GRBINDEX=${GRBINDEX:-/gpfs/gp1/nco/ops/nwprod/util/exec/grbindex}
export GRB2INDEX=${GRB2INDEX:-/gpfs/gp1/nco/ops/nwprod/util/exec/grb2index}
#export WGRIB2=${WGRIB2:-/meso/save/Eric.Rogers/utils/wgrib2_wesley_fornam/wgrib2new}
export WGRIB2=${WGRIB2:-/gpfs/hps/emc/meso/noscrub/Corey.Guastini/nwprod/rap.v3.0.0/exec/rap_wgrib2}
export PARMrap=${PARMrap:-/gpfs/hps/emc/meso/noscrub/Corey.Guastini/nwprod/rap.v3.0.0/parm}
export utilexec=/gpfs/gp1/nco/ops/nwprod/util/exec

cp ${COMOUT}/rap.t${cyc}z.wrfnatf${fhr}.grib2 WRFNAT${fhr}.grib2

#cp ${FIXrap}/rap_conus_elevtiles.grb TOPONDFDCS
#cp ${FIX2rap}/rap_conus_slmask.grb LANDNDFDCS
#cp ${FIXrap}/rap_terrain_g187.grib1 TOPONDFDCS
#cp ${FIXrap}/rap_mask_g187.grib1 LANDNDFDCS
cp ${FIXrap}/rap_conus_elevtiles.grb TOPONDFDCS
...
# Create subset of fields to be posted
    ${WGRIB2} -inv tmp_${grid}.inv ${COMOUT}/rap.${cycle}.wrfnatf${fhr}.grib2 &
done
wait
...
eval grid_specs=\${grid_specs_${grid}}
    grep < tmp_${grid}.inv "`cat ${PARMrap}/rap_smartparms`" | ${WGRIB2} -i ${COMOUT}/rap.${cycle}.wrfnatf${fhr}.grib2 -grib tmp1_${grid}.grib2 &
done
wait
...
# Interpolate fields to new grid
    ${WGRIB2} tmpuv1_${grid}.grib2 -set_bitmap 1 -set_grib_type c3 -new_grid_winds grid \
...
EOF
export err=$?; #err_chk
$utilexec/cnvgrib -g12 RAPCS${fhr} RAPCS${fhr}.grib2
...
export err=$?; #err_chk
$utilexec/cnvgrib -g12 RAPAK${fhr} RAPAK${fhr}.grib2
...
EOF
export err=$?; #err_chk
$utilexec/cnvgrib -g12 RAPPR${fhr} RAPPR${fhr}.grib2
...
$utilexec/cnvgrib -g12 RAPAJN${fhr} RAPAJN${fhr}.grib2
...
$utilexec/cnvgrib -g12 RAPHI${fhr} RAPHI${fhr}.grib2
