...
${ECHO} "exrap_prep_smoke.sh.ecf started at `${DATE}`"
...
MCD_FILE=$FIXrap/smoke/MCD12_2013_NA_13km.bin
BIOME_FILE=$FIXrap/smoke/BIOME_NA_13km.bin
...
RNPP_EXEC=$EXECrap/rap_smoke_proc_npp_frp
RMODIS_EXEC=$EXECrap/rap_smoke_proc_modis_frp
RNOAA20_EXEC=$EXECrap/rap_smoke_proc_j01_frp
FRP_EXEC=$EXECrap/rap_smoke_merge_frp
BBM_EXEC=$EXECrap/rap_smoke_fre_bbm
PREP_EXEC=$EXECrap/rap_prep_chem_sources
QC_MODIS_EXEC=$EXECrap/rap_smoke_qc_modis
QC_VIIRS_EXEC=$EXECrap/rap_smoke_qc_viirs
...
$CP -fp "\$5"*_mod_RAP.txt $DATAHOME_PROC/.
EOF
#cp "\$1"*.txt .
...
# suitable for input to exrap_smoke_wrfinput.sh.ecf

cd ${DATAHOME_PROC}

daily13km=${START_JULIAN}${HH}_daily13km.txt
data_in_domain=NO

# Merge files into one
if [[ "$no_inputs" == YES ]] ; then
    echo "No inputs.  Making an empty daily13km file."
    if [[ "$PREP_SMOKE_DEBUG" == "YES" ]] ; then
        exit 2
    fi
    cat /dev/null > "$daily13km"
else
    cat_me=$( ls -1 | sort )
    if [[ "Q$cat_me" == Q ]] ; then
        echo "Inputs were availble, but no data was in the domain.  Making an empty daily13km file."
        cat /dev/null > "$daily13km"
    else
        cat $cat_me > "$daily13km"
        if [[ ! -s "$daily13km" ]] ; then
            echo "Inputs were available, but all processed data files are empty.  The daily13km file is empty too."
        else
            data_in_domain=YES
        fi
    fi
...
${RUNSERIAL} ${FRP_EXEC} ${START_JULIAN}${HH}_daily13km.txt ${START_JULIAN}${HH}_frp13km.txt
${RUNSERIAL} ${BBM_EXEC} ${START_JULIAN}${HH}_frp13km.txt f${START_JULIAN}${HH}_bbm13km.txt ${MCD_FILE} ${BIOME_FILE}
...
if [[ "$SENDCOM" == YES ]] ; then
    cp -fp FIRE-RAP_new-T-${YEAR}-${MONTH}-${DAY}-${HOUR}0000-g1-bb.bin \
        $COMOUT/rap.t${cyc}z.smoke.sources.bin
fi
